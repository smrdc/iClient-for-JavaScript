...
SuperMap.Handler.GraphicObject = SuperMap.Class(SuperMap.Handler, {
    ...
    createFeature: function(pixel) {
        if(this.symbolInfo !== null && this.libID === this.symbolInfo.libID && this.symbolCode === this.symbolInfo.code && this.originalSymbolInfo === this.symbolInfo){
            var lonlat = this.layer.getLonLatFromViewPortPx(pixel);
            var geometry = new SuperMap.Geometry.Point(
                lonlat.lon, lonlat.lat
            );
            this.point = new SuperMap.Feature.Vector(geometry);
            var geoGraphicObject = SuperMap.Geometry.GeoGraphicObject.getGeometry(this.symbolInfo, this.control.layer, this.serverUrl);
            this.plotting = new SuperMap.Feature.Vector(geoGraphicObject);
            geoGraphicObject.feature = this.plotting;
            var featureStyle = SuperMap.Plot.AnalysisSymbol.getStyle(this.symbolInfo);
            if(featureStyle){
                this.plotting.style = SuperMap.Util.copyAttributes(this.plotting.style, featureStyle);
            }
            ...
            function getCompleted(result){
                ...
                this.originalSymbolInfo = result.originResult;
                var lonlat = this.layer.getLonLatFromViewPortPx(pixel);
                var geometry = new SuperMap.Geometry.Point(
                    lonlat.lon, lonlat.lat
                );
            ...
            getSymbolInfoParams.code = this.symbolCode;
    ...
    modifyFeature: function(pixel) {
        ...
        //    this.clearSelcetFeaturesInLayer();
        //}
        //新建标绘扩展符号
        ...
        if(this.point && this.plotting) {
            ...
            if(this.isDrawing == true){
                ...
                //重新设置标绘扩展符号的控制点
                this.plotting.geometry.controlPoints = this.cloneControlPoints(cp);

                if(this.plotting.geometry.symbolType === SuperMap.Plot.SymbolType.DOTSYMBOL){
                    this.plotting.geometry.locationPoints = this.cloneControlPoints(cp);
                    //重新计算标绘扩展符号的geometry
    ...
    geometryClone: function() {
        ...
        if(geom && (geom.controlPoints || geom.locationPoints)){
            var geo =  geom.clone();

            var feature = new SuperMap.Feature.Vector(geo);
            if(this.plotting.style)
                feature.style = SuperMap.Util.copyAttributes(feature.style, this.plotting.style);
            geo.feature = feature;
            geo.calculateParts();
    ...
    ///**
    // * Method: clearSelcetFeaturesInLayer
    // * 清空图层的选中对象.
    // *
    // */
    //clearSelcetFeaturesInLayer : function(){
    //    if(null === this.layer.map)
    //    {
    //        return;
    //    }
    //
    //    for(var i = 0; i < this.layer.map.controls.length; i++)
    //    {
    //        var control = this.layer.map.controls[i];
    //        if(null === control)
    //        {
    //            continue;
    //        }
